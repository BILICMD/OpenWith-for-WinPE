...
static int dpi_px(int value) {
    return (int)(((long long)value * (long long)g_dpi + 48LL) / 96LL);
}
...
static void start_scroll_timer(HWND hwnd) {
    typedef unsigned long long (WINAPI *PFN_SETTIMER)(HWND,unsigned long long,UINT,void*);
...
static void stop_scroll_timer(HWND hwnd) {
    typedef BOOL (WINAPI *PFN_KILLTIMER)(HWND,unsigned long long);
    HMODULE user = LoadLibraryW(L"user32.dll");
    if (!user) return;
...
static int get_scrollbar_geometry(RECT *track, RECT *thumb) {
    ...
    int thumbH = (int)(((long long)trackH * (long long)viewH) / (long long)contentH);
    if (thumbH < PX(24)) thumbH = PX(24);
    if (thumbH > trackH) thumbH = trackH;
    int travel = trackH - thumbH;
    int thumbTop = tr.top;
    if (travel > 0)
        thumbTop += (int)(((long long)travel * (long long)g_scroll + maxScroll / 2) / maxScroll);
...
static void set_scroll_from_thumb_top(int thumbTop) {
    ...
    g_scroll = (int)(((long long)pos * (long long)maxScroll + travel / 2) / travel);
...
static LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_DPICHANGED: {
        ...
        if (newDpi != g_dpi) {
            ...
            g_dpi = newDpi;
            g_scroll = clamp_scroll_pos((int)(((long long)oldScroll * newDpi + oldDpi / 2) / oldDpi));
            g_scrollTarget = clamp_scroll_pos((int)(((long long)oldTarget * newDpi + oldDpi / 2) / oldDpi));
            g_scrollDragging = 0;
            g_scrollDragOffset = 0;
            stop_scroll_timer(hwnd);
            recreate_fonts();
